...
> 
> 明确 PHP 从**动态脚本语言**约束成**受控运行时语言**，开发者不应将**预启动阶段**作为关注点。  
> 提供遵循 **PSR 规范** 实现的组件，并通过 **DI 容器** 提供灵活的组件替换与扩展能力。  
...
1. **免环境安装** —— 生成源码与php运行时，无需安装php环境。
2. **自动化启动** —— 框架全权接管生命周期。
3. **原生 AOP 支持** —— 面向切面编程的优雅实现。
4. **以 Swoole 为核心** —— 拥抱异步、协程与高并发。
5. **多协议统一支持** —— HTTP、WebSocket、TCP、UDP、MQTT 等协议无缝接入。
